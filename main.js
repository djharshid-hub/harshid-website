...
async function loadSiteData() {
  ...
  applyEventsToPage(stillUpcoming, allPast);
  ...
  if (upEl && upcoming.length) {
    upEl.innerHTML = upcoming.map(ev => `
      <div class="event-card upcoming-card fade-in">
        ...
        ${ev.ticketLink ? `<div class="event-card-footer"><a href="${ev.ticketLink}" class="btn-primary event-ticket-btn" target="_blank" rel="noopener">Get Tickets</a></div>` : ''}
      </div>
    `).join('');
